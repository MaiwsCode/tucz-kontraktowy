...
<div style='float:left;margin-bottom:25px;'>
     ...
     <a class='button'  {$hrefMonthView}> Tydzień </a> 
     <a class='button' {$hrefTypeView}> {$typeNameChange} </a>
     <a class='button' {$prevMonth}> << </a>
     <a class='button' {$currentMonth}> * </a>
     <a class='button' {$nextMonth}> >> </a>
</div>
...
<table cellspacing='0' style='width:100%;font-size:14px;' id='raport'>
    ...
    <tr>
        ...
        {foreach from=$months item=month key=key name=name}
            {if $month.current}
                <th class='header' style='color:red;'> Miesiąc {$month.month} </th>
            {else}
                <th class='header'> Miesiąc {$month.month} </th>
            {/if}
        {/foreach}
    </tr>
    {foreach from=$contracts item=contract key=key name=name}
        <tr>
            ...
            {foreach from=$contract.months item=i key=k name=n}
                 <td class='cell'>{$i}</td>
            {/foreach}
